...
The docs shown on TestPyPI (including this README) come from the package you upload. To push the latest package (and thus the latest docs) to TestPyPI:

1. **Bump the version** – TestPyPI does not allow re-uploading the same version. Update the version in both places:
   - `publicai/version.py` → set `__version__ = "0.1.1"` (or your next version)
   - `pyproject.toml` → set `version = "0.1.1"` under `[project]`
...
4. When prompted, use your TestPyPI username and a [TestPyPI API token](https://test.pypi.org/manage/account/token/) (not your password).

To install from TestPyPI after uploading:
```bash
pip install --index-url https://test.pypi.org/simple/ publicai-api
```
